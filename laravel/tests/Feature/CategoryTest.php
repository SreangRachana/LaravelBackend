...
class CategoryTest extends TestCase
{
     ...
     * Test Steps:
     *   1. Create a test category in the database.
     *   2. Call the get category by ID endpoint with the test category's ID.
     *   3. Assert that the response status is 200.
     *   4. Assert that the response contains the correct category data and a success message.
     * Test Data: Auto-generated using factory (Faker).
     * Expected Result: The response should return status 200 and contain the category data and success message.
     * Actual Result: The response status is 200 and contains expected data.
     * Status: Passed
     * Remark: None
     */
     ...
     * Test Steps:
     *   1. Create a category with an initial name.
     *   2. Send a PUT request to update the category's name.
     *   3. Assert that the response status is 200.
     *   4. Assert that the success message is returned.
     *   5. Confirm that the database reflects the updated name.
     * Test Data: Initial name: "Old Name", Updated name: "Updated Name".
     * Expected Result: The category name should be updated in the database and return a success message.
     * Actual Result: The response status is 200 and the database reflects the updated name.
     * Status: Passed
     * Remark: None
     */
     ...
     * Test Steps:
     *   1. Create a category in the database.
     *   2. Send a DELETE request to the category delete endpoint.
     *   3. Assert that the response status is 200.
     *   4. Assert that the success message is returned.
     *   5. Verify that the category no longer exists in the database.
     * Test Data: Auto-generated category with name "To Be Deleted".
     * Expected Result: The category should be removed from the database with a success message.
    ...
    public function test_if_we_can_delete_category(): void
    {
        ...
        // Assert that the response is successful and the deletion message is returned
        $response->assertStatus(200)
                ->assertJsonFragment(['message' => 'Category Deleted!!!']);

        // Assert that the category still exists in the database (because it is soft deleted)
        $this->assertDatabaseHas('categories', [
            'id' => $category->id,
        ]);

        // Check that the 'deleted_at' field is not null (soft delete behavior)
        $softDeletedCategory = Category::withTrashed()->find($category->id);
        $this->assertNotNull($softDeletedCategory->deleted_at);
    }
}
